...
- [ ] **头部按钮重排**：按频率与语义分组、加分隔线——打开（低频，靠左）；右侧 新建分支 / Pull /
      Push / Review ｜ 刷新 / 设置。↑N 徽标点击改为小下拉（AI Review / 放弃未推送提交），
      撤掉独立 Review 按钮：徽标回归"指示 + 汇总入口"，破坏动作藏进菜单且仍有确认框兜底
- [ ] **左侧栏交互习惯**：分支双击切换改**单击切换**（对齐 VS Code / GitHub Desktop 惯例，
      当前分支点击无动作）；组头 hover 图标（标签组的 AI/+）迁入右键菜单
- [ ] **文件/历史行打开方式**：双击改**单击**打开 diff（hover 图标已 stop 传播不冲突；
      误开成本低 Esc 即关），全应用统一为「单击看内容、右键看操作」
- [ ] **三栏与状态栏走查**：左右栏拖宽把手/最小宽度、空态文案位置与措辞、状态栏信息密度、
      提交区复查（M9 已收纳过一轮）
...
- [ ] **浮层键盘统一**：Esc 关闭一切浮层（右键菜单/stash 菜单/分组下拉/切换器，确认框=取消）；
      ...
      右键菜单项支持 ↑↓ 移动 + 回车执行
- [ ] **共享右键菜单组件 + 左栏操作迁移**：抽 ui/ContextMenu（替换 tab/提交/文件/分组/stash 五处
...
- [ ] **规范**：tooltip/hover 一行、动词开头、只说「动作 + 结果」不说实现细节与后果
      （后果留给确认框讲）；空态必须给出下一步动作；同一动作全应用只用一种说法
- [ ] **全量走查**：逐条过 tooltip / 确认框 / 空态 / 右键菜单 / humanize 文案。
